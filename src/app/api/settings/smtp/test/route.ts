...
export async function POST(request: Request) {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"${settings.fromName || 'Culture Survey'}" <${settings.fromAddress}>`,
      to: settings.fromAddress,
      subject: 'Culture Survey — SMTP Test Email',
      html: `
        <div style="font-family: Arial, sans-serif; padding: 24px;">
          ...
          <p style="color: #6b7280; font-size: 13px;">Sent from Culture Survey at ${new Date().toISOString()}</p>
        </div>
      `,
    });
